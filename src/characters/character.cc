...
std::shared_ptr<std::mt19937> Character::rand_gen;
...
RealCharacter::attack(Character * target) {
    ...
    if (distrib(*(Character::rand_gen)) == 0) strategy->attack(target);
...
CStatus     RealCharacter::default_status() { return {default_hp, default_atk, default_def}; }
