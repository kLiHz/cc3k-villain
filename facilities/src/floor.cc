...
void Floor::initialize() {
    ...
    // 2. Generate charaters, push'em into each chamber;
    Character * t_ch;
    for (int i = 0; i < 2; ++i ) {
        t_ch = new Elf();
        while (!chambers[rand() % chambers.size()]->push(t_ch));
        t_ch = new Orc();
        while (!chambers[rand() % chambers.size()]->push(t_ch));
        t_ch = new Merchant();
        while (!chambers[rand() % chambers.size()]->push(t_ch));
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        while (!chambers[rand() % chambers.size()]->push(t_potion));
    }
    Item * t_gold;
    for (int i = 0; i < 10; ++i) {
        int rand_num = rand() % 8;
    ...
    // 3. Generate stair.
    while (true) {
        int room_num = rand() % chambers.size();
